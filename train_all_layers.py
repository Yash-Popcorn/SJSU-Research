...
def modify_weights(weights, binary_data, num_bits_per_weight):
    binary_data_gen = itertools.cycle(bit for bit in binary_data)
...
with h5py.File('smalldata.hdf5', 'r') as f:
...
model = Sequential()
model.add(LSTM(512, input_shape=(200, 1), recurrent_dropout=0.2))  # Adding dropout to LSTM layer
...
new_weights = modify_weights(weights, binary_data, 22)
...
# Model Training
batch_size = 32
train_generator = data_generator("smalldata.hdf5", train_indices, batch_size=batch_size)
val_generator = data_generator("smalldata.hdf5", val_indices, batch_size=batch_size)
